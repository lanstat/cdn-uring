    ...
    if (event.events & EPOLLIN) {
        ...
        do {
            ...
            if (socket == IPv6Socket) {
                ...
                size = recvfrom(socket->getFD(), buffer, sizeof(buffer), 0,
...
void Dns::cleanCache() {
    ...
    const std::map<uint64_t /*outdated_date in s from 1970*/,
...
bool Dns::getAAAA(Http *client, const std::string &host, const bool &https) {
    ...
    if (client == nullptr) {
        std::cerr << __FILE__ << ":" << __LINE__
                  << " Dns::get() client==nullptr" << std::endl;
        abort();
    }
    ...
    while (serverDNSindex < dnsServerList.size()) {
        ...
        if (dnsServer.mode == Mode_IPv6) {
            const int result =
                sendto(IPv6Socket->getFD(), &buffer, pos, 0,
        ...
        {
            const int result =
                sendto(IPv4Socket->getFD(), &buffer, pos, 0,
