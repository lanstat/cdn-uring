...
const unsigned char Dns::exclude[] = {0x28, 0x03, 0x19, 0x20, 0x00, 0x00, 0x00,
                                      0x00, 0xb4, 0xb2, 0x5f, 0x61, 0xd3, 0x7f};
...
void Dns::parseEvent(const int &event, int socket) {
    if (true) {
        int size = 0;
        do {
            char buffer[1500];
            ...
            if (queryList.find(transactionId) != queryList.cend()) {
                ...
                const std::vector<DnsRequest*> &http = q.http;
                const std::vector<DnsRequest*> &https = q.https;
                        ...
                        while (answersIndex < answers) {
                            ...
                            switch (type) {
                                ...
                                case 0x001c: {
                                    ...
                                    // TODO saveToCache();
                                    if (memcmp(buffer + pos, Dns::include,
                                               sizeof(Dns::include)) != 0 ||
                                        memcmp(buffer + pos, Dns::exclude,
                                               sizeof(Dns::exclude)) == 0) {
                                        if (!clientsFlushed) {
                                            clientsFlushed = true;
                                            addCacheEntry(
...
void Dns::addCacheEntryFailed(const StatusEntry &s, const uint32_t &ttl,
...
void Dns::addCacheEntry(const StatusEntry &s, const uint32_t &ttl,
...
bool Dns::read8Bits(uint8_t &var, const char *const data, const int &size,
                    int &pos) {
...
bool Dns::read16Bits(uint16_t &var, const char *const data, const int &size,
                     int &pos) {
    uint16_t t = 0;
...
bool Dns::read16BitsRaw(uint16_t &var, const char *const data, const int &size,
                        int &pos) {
    if ((pos + (int)sizeof(var)) > size) return false;
...
bool Dns::read32Bits(uint32_t &var, const char *const data, const int &size,
                     int &pos) {
    if ((pos + (int)sizeof(var)) > size) return false;
    uint32_t t;
...
bool Dns::GetAAAA(DnsRequest *request, const std::string &host,
                  const bool &https) {
    if (dnsServerList.empty()) {
        std::cerr << "Sorry but the server list is empty" << std::endl;
        abort();
    ...
    if (queryListByHost.find(host) != queryListByHost.cend()) {
        ...
        if (queryList.find(queryId) != queryList.cend()) {
            ...
            if (q.host == host) {
                if (q.retryTime >= Dns::retryBeforeError() &&
...
void Dns::removeQuery(const uint16_t &id, const bool &withNextDueTime) {
    ...
    if (queryByNextDueTime.find(query.nextRetry) == queryByNextDueTime.cend())
        std::cerr << __FILE__ << ":" << __LINE__ << " query " << id
                  << " not found into queryList" << std::endl;
    queryList.erase(id);
    if (httpInProgress > 0) httpInProgress--;
}
    ...
    if (queryListByHost.find(host) != queryListByHost.cend()) {
        ...
        if (queryList.find(queryId) != queryList.cend()) {
            ...
            std::cerr << __FILE__ << ":" << __LINE__
                      << " try remove: " << client << " to \"" << host
                      << "\" but queryListByHost seam wrong" << std::endl;
            abort();
        }
    } else {
...
int Dns::requestCountMerged() { return queryListByHost.size(); }
...
uint8_t Dns::serverCount() const { return dnsServerList.size(); }
*/

uint8_t Dns::retryBeforeError() const { return 4; }

uint8_t Dns::resendQueryDNS_ms() const { return 200; }
...
void Dns::checkQueries() {
    ...
    for (auto const &x : queryByNextDueTime) {
        const uint64_t t = x.first;
        if (t > Backend::msFrom1970()) return;
        ...
        for (auto const &id : list) {
            ...
            if (query.retryTime >= Dns::retryBeforeError() &&
                ...
                if (cacheAAAA.find(query.host) != cacheAAAA.cend()) {
                    ...
                    if (entry.status == StatusEntry_Right) {
                        ...
                        if (!http.empty()) {
                            memcpy(&targetHttp.sin6_addr, &entry.sin6_addr, 16);
                            for (Http *const c : http) c->dnsRight(targetHttp);
                        }
                        cacheFound = true;
                    }
                }
                if (cacheFound == false) {
                    for (Http *const c : http) c->dnsError();
                    for (Http *const c : https) c->dnsError();
            ...
            while (serverDNSindex < dnsServerList.size()) {
                ...
                if (dnsServer.mode == Mode_IPv6) {
                    const int result =
                        sendto(IPv6Socket->getFD(), query.query.data(),
                               query.query.size(), 0,
                               (struct sockaddr *)&dnsServer.targetDnsIPv6,
                               sizeof(dnsServer.targetDnsIPv6));
                ...
                {
                    const int result =
                        sendto(IPv4Socket->getFD(), query.query.data(),
                               query.query.size(), 0,
                               (struct sockaddr *)&dnsServer.targetDnsIPv4,
                               sizeof(dnsServer.targetDnsIPv4));
                    if (result != (int)query.query.size()) {
        ...
        if (query.retryTime >= Dns::retryBeforeError()) {
            ...
            if (cacheAAAA.find(query.host) != cacheAAAA.cend()) {
                ...
                if (entry.status == StatusEntry_Right) {
                    if (!https.empty()) {
                        memcpy(&targetHttps.sin6_addr, &entry.sin6_addr, 16);
                        for (Http *const c : https) c->dnsRight(targetHttps);
                    }
                    if (!http.empty()) {
                        memcpy(&targetHttp.sin6_addr, &entry.sin6_addr, 16);
                        for (Http *const c : http) c->dnsRight(targetHttp);
...
void Dns::dnsRight(const sockaddr_in6 &sIPv6) {
    char astring[INET6_ADDRSTRLEN];
    inet_ntop(AF_INET6, &(sIPv6.sin6_addr), astring, INET6_ADDRSTRLEN);
    std::cout << "LAN_[" << __FILE__ << ":" << __LINE__ << "] " << astring
              << std::endl;
}
...
uint64_t Dns::msFrom1970()  // ms from 1970
{
    struct timeval te;
...
in6_addr *Dns::GetEntry(const std::string &host) {
    if (cacheAAAA.find(host) != cacheAAAA.cend()) {
        CacheAAAAEntry &entry = cacheAAAA.at(host);
